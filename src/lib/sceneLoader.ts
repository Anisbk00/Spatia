...
function parsePlyFile(buffer: ArrayBuffer): SplatData {
  ...
  if (isBinary) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const rIdx = propIndex("f_dc_0") ?? propIndex("red") ?? propIndex("r");
      const gIdx = propIndex("f_dc_1") ?? propIndex("green") ?? propIndex("g");
      const bIdx = propIndex("f_dc_2") ?? propIndex("blue") ?? propIndex("b");
      if (rIdx >= 0) {
        let r = values[properties[rIdx].name];
        let g = values[properties[gIdx >= 0 ? gIdx : rIdx].name];
        let b = values[properties[bIdx >= 0 ? bIdx : rIdx].name];
        // SH DC to color: C = 0.5 + SH_C * sh2rgb
        const SH_C = 0.28209479177387814;
        if (properties[rIdx].name === "f_dc_0") {
      ...
      if (opIdx >= 0) {
        ...
        if (properties[opIdx].name === "opacity") {
          op = 1 / (1 + Math.exp(-op));
        } else {
          op = op / 255;
        }
 ...
 */
export async function loadScene(modelUrl: string): Promise<SplatData> {
...
export async function loadSceneProgressive(
  ...
  if (!response.body || !contentLength) {
    // No streaming support, fall back to regular load
    onProgress?.(1, 1);
    return loadScene(modelUrl);
  }
