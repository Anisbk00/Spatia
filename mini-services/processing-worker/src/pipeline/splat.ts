...
import { createSeededRandom } from "./stages";
...
export async function runSfMReconstruction(
  ...
  const imageCount = Number(
    ctx.artifacts.valid_image_count || ctx.imageUrls.length
  );

  // Use seeded PRNG for deterministic results
  const random = createSeededRandom(ctx.sceneId);
  ...
  logs.push(
    `[${new Date().toISOString()}] Extracting ~${totalFeatures.toLocaleString()} features from ${imageCount} images...`
  );
  ...
  logs.push(
    `[${new Date().toISOString()}] Matching features across ${matchPairs} image pairs (~${expectedMatches.toLocaleString()} matches)...`
  );
  ...
  for (let i = 0; i < imageCount; i++) {
    ...
    const radius = 2 + random() * 2; // Variable radius for more realistic placement
    cameraPoses.push({
      position: [
      ...
      fov: 60 + Math.floor(random() * 20), // 60-80 FOV range
    });
  }
  logs.push(
    `[${new Date().toISOString()}] Estimated ${cameraPoses.length} camera positions`
  );
  ...
  logs.push(
    `[${new Date().toISOString()}] Match coverage: ${(matchCoverage * 100).toFixed(1)}%`
  );
  logs.push(
    `[${new Date().toISOString()}] Quality score: ${qualityScore.toFixed(2)}`
  );
